    ...
    process(msg) {
        if (!this.isLocked(msg)) {
            this.dataStore.getTeams().subscribe(teams => {
                ...
                const maxNumberLength = Math.max(...(topTeams.map(t => t.wins + t.losses)));
                const message = topTeams.reduce((msg, team) => {
                    ...
                    return msg + winrate + '% | ' + DiscordUtils_1.DiscordUtils.fillWithSpaces(String(team.losses + team.wins), String(maxNumberLength).length) + ' | ' + team.name + '\n';
